Unit ac_Strings; (* (C)opyright 1995-2003 George Birbilis / Agrinio Club *)
...
(************************************************************************
 History:
  ...
  10Dec2002 - added "copyAfterChar" function
   1Jul2003 - added "FindCharsRight", "FindNonCharsRight" and "removeCharPos" functions
...
function FindChars(s,matchChars:string;offsetLeft:integer):integer; //find the first char from the left that is one of...
function FindCharsRight(s,matchChars:string;offsetRight:integer):integer; //find the first char from the right that is one of...
function FindNonChars(s,notWantedChars:string;offsetLeft:integer):integer; //find the first char from the left that isn't one of...
function FindNonCharsRight(s,notWantedChars:string;offsetRight:integer):integer; //find the first char from the left that isn't one of...
...
function upTo(s:string;last:integer):string; //maybe should renamed this to "fromStart"
function removeCharPos(s:string;pos:integer):string;
...
begin
 if offsetLeft>0 then
  for i:=offsetLeft to length(s) do
...
function FindCharsRight;
var i:integer;
begin
 if offsetRight<=length(s) then
  for i:=offsetRight downto 1 do
   if pos(s[i],matchChars)<>0 then
    begin
    FindCharsRight:=i;
    exit;
...
function removeCharPos(s:string;pos:integer):string;
begin
 result:=upTo(s,pos-1)+toEnd(s,pos+1);
end;
